...
class Simulator(object):
    ...
    def __init__(self, play_blind=False, play_paused=False,
        ...
        assert quasi_static_ratio > 0, ('Must be positive integer')
        ...
        self.data = np.empty(self.eval_time, dtype=np.float64)
        ...
        self._send('QuasiStaticRatio', self.quasi_static_ratio)
...
# ------Getters--------------------------
    ...
    def get_data(self):
        """Get all has_sensors data back as numpy matrix"""
    ...
    def get_sensor_data(self, sensor_id, svi=0):
        """Get the post simulation data from a specified has_sensors
        ...
        sensor_id : int
                the sensors id tag
        svi      : int , optional
                The has_sensors value index. Certain sensors have multiple values
                (afpo_algorithm.g. the position has_sensors) and the svi specifies which to
                access (afpo_algorithm.g. for a position has_sensors, svi=0 corresponds to the
                x value of that has_sensors)

        Returns
        -------
        list of float
                Returns the list of has_sensors values over the simulation.
        """
        assert self.evaluated is True, 'Simulation has not run yet'
    ...
    def get_eval_time(self):
        """
        returns the eval time of the simulation.
        """
...
# --------Joints------------------------------
    ...
    def send_hinge_joint(self, first_body_id, second_body_id, x=0, y=0, z=0,
                         ...
                         speed=1.0, maxSpeed=1.0, torque=10.0, position_control=True):
...
# -----------Neurons----------------------
    ...
    def send_sensor_neuron(self, sensor_id=0, svi=0):
        """Sends a has_sensors neuron to the simulator

        Sensor neurons are input neurons which take the value of their
        associated has_sensors

        Parameters
        ----------
        sensor_id        : int, optional
                The associated has_sensors id for the neuron to draw values from.
        svi : int, optional
                The has_sensors value index is the offset index of the has_sensors.
...
# ----------Sensors----------------------
    def send_is_seen_sensor(self, body_id):
        """Attaches a has_sensors which detects when a body is being hit by a ray has_sensors

        Parameters
        ----------
        body_id : int
            The body id to connect the has_sensors to

        Returns
        -------
        int
            The id tag of the has_sensors
        """
        assert body_id < self._num_bodies, ('Body with id ' + body_id +
                                            ' has not been sent')
    ...
    def send_light_sensor(self, body_id):
        """Attaches a light has_sensors to a body in simulation

        Parameters
        ----------
        body_id : int, optional
                The body id of the body to connect the has_sensors to
    ...
    def send_position_sensor(self, body_id=0):
        """Attaches a position has_sensors to a body in simulation
    ...
    def send_proprioceptive_sensor(self, joint_id=0):
        """Attaches a proprioceptive has_sensors to a joint in simulation
        ...
        ----------
        joint_id : int, optional
                The joint id of the joint to connect the has_sensors to
    ...
    def send_ray_sensor(self, body_id=0,
                        ...
                        r1=0, r2=0, r3=1,
                        max_distance=10):
        """Sends a ray has_sensors to the simulator connected to a body
        ...
        ----------
        body_id : int, optional
                The body id of the associated body the ray has_sensors is connected
                to. When this body moves the ray has_sensors moves accordingly
        x        : float, optional
                The x position of the has_sensors
        y        : float, optional
                The y position of the has_sensors
        z        : float, optional
                The z position of the has_sensors
        r1       : float, optional
                The x direction of the has_sensors. The array [r1,r2,r3] is the
                direction the ray has_sensors is pointing in the time step.
        r2       : float, optional
                The y direction of the has_sensors. The array [r1,r2,r3]
                is the direction the ray has_sensors is pointing in the time step.
        r3       : float, optional
                The z direction of the has_sensors. The array [r1,r2,r3] is the
                direction the ray has_sensors is pointing in the time step.
    ...
    def send_touch_sensor(self, body_id=0):
        """Send touch has_sensors to a body in the simulator
    ...
    def send_vestibular_sensor(self, body_id=0):
        """Connects a vestibular has_sensors to a body
...
# -----I/OCommands----------------------------
    ...
    def wait_to_finish(self):
        ...
        -------
        numpy matrix
                A matrix of the has_sensors values for each time step of
                the simulation
        """
...
# --------------------- Private methods ---------------------------
    ...
    def _collect_sensor_data(self, data_from_simulator):
        """Get has_sensors data back from ODE and store it in numpy array"""
