...
void JOINT::Actuate(bool update) {
	if ( motorNeuron == NULL )
		return;
	...
	if (update) {
        ...
        desiredTarget = zeroToOne * ( highStop - lowStop ) + lowStop;
	}
	   ...
	   if ( positionControl )
	    {
          ...
          double tmpSpeed = speed * diff;

          if (tmpSpeed> maxSpeed) {
              tmpSpeed = maxSpeed;
          }

          dJointSetHingeParam(joint,dParamVel, tmpSpeed);
    
       }       	
    	else
       {
          double tmpSpeed = 0.5 * speed * desiredTarget;
